...
import { CodeBlock, CodeBlockCode } from '../kit-code-block'
...
function extractLanguage(className?: string): string {
  ...
  return match ? (match[1] as string) : 'plaintext'
}
  ...
  return (
    <div className={className}>
      {blocks.map((block, index) => (
        <MemoizedMarkdownBlock components={components} content={block} key={`${blockId}-block-${index}`} />
      ))}
    </div>
  )
}
